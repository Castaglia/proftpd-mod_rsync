...
sub rsync_list_empty_dir {
  ...
  my $setup = test_setup($tmpdir, 'rsync');
  ...
  if ($pid) {
    eval {
      ...
      my $client = File::RsyncP->new({
        ...
        rsyncCmd => "ssh -2 -4 -a -q -x -oIdentityFile=$priv_key -oPasswordAuthentication=no -oPubkeyAuthentication=yes -oStrictHostKeyChecking=no -oCheckHostIP=no -p $port $setup->{user}\@127.0.0.1 rsync",
        rsyncArgs => [
        ],
      });
